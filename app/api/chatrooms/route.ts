...
export async function POST(req: NextRequest) {
    try {
        ...
        if (reqBody.type === 'private') {
            const opponentUser = await db.select().from(users).where(eq(users.id, reqBody.opponentUser))
        ...
        const chatroom: Chatroom[] = await db.insert(chatrooms).values({
        ...
        if (reqBody.type === 'private') {
            const opponentUser: ChatroomUser[] = await db.insert(chatroomUsers).values({
                chatroomId: chatroom[0].id,
