...
export default function GroupChatCardList({ userId }: Props) {
  ...
  useEffect(() => {
    ...
    setJoinedChatrooms(chatroomsData.filter((chatroom) => (chatroom.id % 2 == 1)));
    setNotJoinedChatrooms(chatroomsData.filter((chatroom) => (chatroom.id % 2 == 0)));
  }, []);
        ...
        <div className="text-slate-500 flex flex-col gap-2 mt-[10px]">
...

          {
            joinedChatrooms.map((chatroom, index) => (
              <JoinedGroupChatCard key={index} chatroom={chatroom} handleLeaveChat={handleLeaveChat} />
            ))
          }
        ...
        <div className="text-slate-500 mt-[25px] flex flex-col gap-2">
          ...
          </div>
          {
            notJoinedChatrooms.map((chatroom, index) => (
              <NotJoinedGroupChatCard key={index + joinedChatrooms.length} chatroom={chatroom} handleJoinChat={handleJoinChat} />
            ))
          }
        </div>
      </div>
